    ...
    class DemoComposerVC: ComposerVC {
        ...
        override func createNewMessage(text: String) {
            ...
            guard let navController = parent?.parent as? UINavigationController,
                  let controller = channelController else { print(channelController); return }
//            var num = Int(text)
//            if num != nil {
//                let demoUser: DemoUserType = DemoUserType.credentials(UserCredentials(id: text, name: text, avatarURL: URL(string:"https://cdn.iconscout.com/icon/free/png-256/free-nurse-108-444876.png")!, token: .development(userId: text), birthLand: "Earth"))
//                
//                
                let currId = StreamChatWrapper.shared.client?.currentUserId
                print("currId Composer VC")
                print(currId)
                let userInfo: UserInfo = .init(id: currId!)
//
//                
//                StreamChatWrapper.shared.connect(user: demoUser, completion: { [weak self] error in
//                    if let error = error {
...
//                })
//                
//                StreamChatWrapper.shared.client?.connectUser(userInfo: userInfo, token: .development(userId: currId!))
//                
//            }
            print(1)
...
//            }
//        }
        searchController.search(query: .init(filter: .equal("is_real", to: true)))
...
//            cell.nameLabel.text = "Send SMS"
//            cell.descriptionLabel.text = "Type in a number"
//            cell.isUserInteractionEnabled = false
//            if (currId == "jsoyinka" || currId == "test") {
...
//        }
        ...
        // Select user
//        if (!searchField.hasText && selectedUserIds.isEmpty) {
//            return
//        }
        
//        var response : ConnectedUser
//        do { response = try await searchController.client.connectUser(userInfo: userInfo, token: .development(userId: searchField.text!))
//            return
//        } catch {
...
//      Select user
        if (!searchField.hasText && (cell.user?.id.isEmpty) == nil) {
            print(searchField.textualRange.isEmpty)
            return
        }
        print("test norm 1")
//        cell.accessoryImageView.image = UIImage(systemName: "checkmark.circle.fill")
        let token = UISearchToken(
            ...
            text: cell.user?.name ?? cell.user?.id ?? searchField.text!
        )
        print("test norm 2")

        let demoUser: DemoUserType
        if ((cell.user?.id.isEmpty) == nil) {
//            let userInfo = UserInfo(
//                id: searchField.text!,
//                name: searchField.text!,
//                imageURL: URL(string: "https://cdn.iconscout.com/icon/free/png-256/free-nurse-108-444876.png")
//            )
//
...
//            searchController.client.connectUser(userInfo: userInfo, token: .development(userId: searchField.text!))
            

//            searchController.search(term: searchField.text)
            demoUser = DemoUserType.credentials(UserCredentials(id: text!, name: text!, avatarURL: URL(string:"https://cdn.iconscout.com/icon/free/png-256/free-nurse-108-444876.png")!, token: .development(userId: text!), birthLand: "Earth"))
            print(demoUser)
            ...
            StreamChatWrapper.shared.connect(user: demoUser, completion: { [weak self] error in
                if let error = error {
                    ...
                    DispatchQueue.main.async {
                        log.warning(error.localizedDescription)
                        print("Dispatch.main.async")
                    }
                } else {
                    // Allows creation when unblocked
                    let demoUser2 = DemoUserType.credentials(UserCredentials(id: text!, name: text!, avatarURL: URL(string:"https://cdn.iconscout.com/icon/free/png-256/free-nurse-108-444876.png")!, token: .development(userId: text!), birthLand: "Earth"))
                    StreamChatWrapper.shared.connect(user: demoUser, completion: { [weak self] error in
                        if let error = error {
                            log.warning(error.localizedDescription)
                            print("2 let error = error")
                            DispatchQueue.main.async {
                                log.warning(error.localizedDescription)
                                print("2 Dispatch.main.async")
                            }
                        } else {
                            do {
                                token.representedObject = demoUser

                                self!.searchField.replaceTextualPortion(of: self!.searchField.textualRange, with: token, at: self!.searchField.tokens.count)
                                self!.update(for: .selected)
                                print("2 Empty")
                                print("2 SF.text")
                                print(text!)
                                let finalClient = self!.searchController.client
                                self!.composerView.channelController = try finalClient
                                    .channelController(
                                        createDirectMessageChannelWith: Set([text!]),
                                        name: nil,
                                        imageURL: nil,
                                        extraData: [:]
                                    )
...
//                let userInfo: UserInfo = .init(id: currId!)
//                client.connectUser(userInfo: userInfo, token: .development(userId: currId!), completion: { [weak self] error in
//                    if let error = error {
//                        log.warning(error.localizedDescription)
//                        print("3 let error = error")
//                        DispatchQueue.main.async {
//                            log.warning(error.localizedDescription)
//                            print("3 Dispatch.main.async")
//                        }
//                    }
//                })
//                StreamChatWrapper.shared.client?.connectUser(userInfo: userInfo, token: .development(userId: currId!))
