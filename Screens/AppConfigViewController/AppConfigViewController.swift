...
struct DemoAppConfig {
    ...
    /// Panel That helps you login.
    var loginToUser: LoginToUser?
...
class AppConfigViewController: UITableViewController {
    ...
    private func showLoginToUserAlert() {
        let alert = UIAlertController(
            title: "Login To User",
            message: "Input the UserID to Login.",
            preferredStyle: .alert
        )
        ...
        alert.addTextField { textField in
            ...
            textField.autocapitalizationType = .none
            textField.autocorrectionType = .no
            if let userId = self.demoAppConfig.loginToUser?.userId {
                textField.text = userId
            }
        }
        alert.addAction(.init(title: "Enable", style: .default, handler: { _ in
            guard let userId = alert.textFields?[0].text else { return }
            let userInfo: UserInfo = .init(id: userId)
            let demoUser: DemoUserType = DemoUserType.credentials(UserCredentials(id: userId, name: userId, avatarURL: URL(string:"https://as1.ftcdn.net/v2/jpg/03/97/29/00/1000_F_397290052_wm43JsJdX97l09lWUujXN1TYGIUYxXGk.jpg")!, token:  .development(userId: userId), birthLand: "Earth"))
            print(StreamChatWrapper.shared.connect)
            StreamChatWrapper.shared.client?.connectUser(userInfo: userInfo, token: Token.development(userId: userId));
            print(userInfo)
            print(StreamChatWrapper.shared.client?.currentUserId)
//            StreamChatWrapper.shared.client?.connectUser(userInfo: userInfo, token: .development(userId: userId))
        }))
